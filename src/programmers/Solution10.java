...
*/
    public static void main(String[] args){
       ...
       int a =  solution(5,3,2);

        /*
        * 1. 약수의 개수
        * 2. 철 무게
        *
        * number 만큼 포문 돌면서 약수의 개수를 구한뒤 무게에 섬해주기
        * */
    ...
    public static int solution(int number, int limit, int power) {
        ...
        for(int i = 1; i <= number ; i++){
            ...
            if(numberOfDivisor(i) > limit){
                answer += power;
            }else{
                answer += numberOfDivisor(i);
            }
    ...
    public static int numberOfDivisor(int night){

        int count = 0;
        for( int i = 1; i*i <= night ; i++)
        {
            if(i*i==night) count++;
            else if(night % i ==0) count+=2;
        }
        return count;
    }
